...
public class ClientesBD {
    ...
    ArrayList<Cliente> resultado = new ArrayList<>();
        try {
            ...
            try {
                System.out.println("error queery"+ e.getMessage());
                e.printStackTrace();
            } catch (Exception e1) {
                System.out.println("error cierre"+ e1.getLocalizedMessage());
                e1.printStackTrace();
            }
        }
    return resultado;
    ...
    public boolean actualizarCliente(Cliente Cliente, String nitAnterior){
        try {
            String query = "UPDATE TELCLIENTE SET NIT=?, NOMBRES=?, APELLIDOS=?, GENERO=?, DIRECCICON=? WHERE NIT=?";
            PreparedStatement stmt = BD.Conectar().prepareStatement(query);
            stmt.setString(1, Cliente.getNit());
            stmt.setString(2, Cliente.getNombre());
            stmt.setString(3, Cliente.getApellido());
